...
| Role-turn lifecycle verdict and native session liveness | Herdr is authoritative only for exact owned-session/process aliveness. The typed Agent Runtime lifecycle verdict combines native liveness, same-scope claim-lease liveness, and provider-turn state before workflow mutation, claim release, retry, cleanup, or escalation. Malformed, incompatible, unknown, and unreachable native responses fail closed. Cleanup is bounded and verified before claim release; failure quarantines ownership. Dispatch acquires and revalidates ownership inside one same-scope fence. Deterministic hook failures and exhausted transient budgets reconcile one tracker escalation per failure fingerprint and stable retry epoch, with run identity retained only as evidence. | Deterministic public-Interface tests cover every lifecycle matrix cell; normal, abnormal, cancelled, timed-out, brutally terminated, startup-recovery, and orchestrator-restart cleanup paths; bounded cleanup and liveness queries; cleanup quarantine; false-death refusal; final dispatch revalidation and concurrent at-most-one launch; typed hook schema and retry exhaustion; and tracker escalation under repeated delivery, process restart, concurrency, and partial mutation failure through the Memory and Linear Adapters. Corpus obligations for orphan survival, false-death reroute, dual-turn race, lost-transition residue, and silent deterministic retry remain explicit. Run broader `make all` validation because this surface changes shared orchestration, provider transport, tracker, and recovery behavior. | `Agent Fixes` for any durable mutation that bypasses the verdict, claim release before verified absence, unbounded native cleanup/liveness calls, absent-vs-unknown collapse, dispatch revalidation gaps, retry leakage, or duplicate escalation. `Human Escalation` when provider-native liveness evidence and the canonical lifecycle contract irreconcilably conflict. |
