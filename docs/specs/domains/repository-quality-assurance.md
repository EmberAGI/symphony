...
| Irrecoverable runtime failure classification and escalation | Runtime retry policy uses a provider-neutral failure family before ordinary retry scheduling. Deterministic irrecoverable families immediately stop ordinary retry, verify and update same-scope process ownership to blocked, apply or preserve the exact `Human Escalation` tracker label, and move the issue to `Human Escalation` when supported. No Linear comment is required or created. Repeated identical no-progress failures use the bounded fingerprint and three-consecutive-observation threshold from [Agent Runtime](./agent-runtime.md); elapsed wall-clock time is not an eligibility cap. Tracker mutation failures stay blocked and do not re-enter ordinary retry. Status API, dashboard, logs, and process ownership distinguish blocked failures from retry queue entries. Secret-bearing provider payloads, tokens, environment values, full issue bodies, and unbounded process output are never emitted. | Evidence discovery inventories bounded Octo-owned role-run/status logs, process-ownership records, tracker state/labels, local runtime state, redacted incident evidence, and safe local CLI probes when available. Deterministic tests cover provider-adapter normalization, all accepted failure families, repeated-no-progress threshold and reset conditions, transient exclusions, AgentRunner exit normalization, Orchestrator retry suppression and escalation routing, tracker label/state success and failure paths, process-ownership status/API representation, compact run-log summaries, and redaction. Include existing Codex and Claude auth/no-progress/generic retry regressions and at least one real adapter-boundary fixture. Run broader `make all` validation. | `Agent Fixes` for missing families, transient over-classification, retry leakage, delayed escalation, comment traffic, missing status distinction, tracker mutation regressions, redaction gaps, or fixtures that bypass provider parsing. `Human Escalation` when provider/runtime semantics conflict with durable sources or require operator-owned credential/configuration changes to validate. |
