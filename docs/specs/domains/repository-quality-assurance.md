...
role skill packages and manifests. It owns skill discovery, invocation, role
exposure, provider projection, and evaluation behavior. Symphony owns runtime
Adapters and workflow prompt loading, but it does not provide a second editable
skill authority.
...
Changes to cross-provider skill discovery, invocation, role exposure,
projection, or evaluation MUST be implemented and accepted in
...
| Octo role skill sources or provider projections | Symphony contains no Octo role skill source inventory or exposure manifests. Active Symphony workflow, runtime-specification, validator, and test consumers do not resolve repository-local skill paths. `EmberAGI/scaling-octo-engine` remains the only editable authority for discovery, invocation, role exposure, provider projection, and evaluation. | Focused repository tests proving both old tracked inventories are absent and every active Symphony consumer path is gone. Run `make all` because the contract changes the repository acceptance surface. | `Agent Fixes` for any reintroduced source, manifest, consumer, or replacement projection. `Human Escalation` only when the Octo ownership contract conflicts with a required runtime source. |
