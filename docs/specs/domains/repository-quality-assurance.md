...
| --- | --- | --- | --- |
| Herdr live-agent delegation lifecycle | Herdr 0.7.5/protocol 17 remains the deep module that owns atomic terminal input, provider keyboard protocol, prompt acknowledgement, and lifecycle waiting. Symphony creates the workspace pane from returned IDs and uses strict named/kind `agent start`. Every orchestrator and worker launch first persists the session wrapper path in the pane, preflights exact non-interactive kind resolution, and starts only through one token-named `--symphony-launch-projection`; the fail-closed wrapper validates canonical containment, ownership, and mode 0500, records the observed pinned unattended flag as diagnostic evidence, and requires exact token-bound wrapper/projection acknowledgements. Launch failures stay distinct typed stages. Participant messages use verified native `agent prompt`; prompts request the upstream terminal set plus `working` with an effective 5001 ms floor and at most two typed stall recoveries. All five statuses (`idle`, `working`, `blocked`, `done`, `unknown`) are typed everywhere they are read: terminal waits request exactly `idle, done, blocked`, `blocked` settles as non-success, `working` and `unknown` never prove completion, and out-of-enum values are protocol/version failures distinct from command failures. Turn completion is observed through the bounded, idempotent EMB-1244 Stage 2 supervision machine: typed `agent get` status reads on a 30s/5s/0ms-jitter cadence, an observable progress cursor distinct from `working` status, bounded unknown/read-failure retries, bounded stale-working recovery via the server-owned terminal wait, blocked outcomes preserved and surfaced within one interval plus one read timeout, and hard-budget checkpoint-and-preserve; a typed work-preservation checkpoint failure blocks destructive shutdown, and no new store, outbox, run log, or configuration surface backs any of it. The worker launcher accepts only the returned pane and strict worker name, while the worker projection permits only agent list/get/read/prompt/wait and denies descendant start, raw input, key injection, topology, and server control. No 0.7.4 prompt-submission pane run, manual confirmation, revision-acknowledgement choreography, top-level wait, dual-version, translation, or compatibility path remains; `pane run` exists only for launch preparation and preflight, and read-only revision observation for the prompt-transition settle guard is permitted. | Deterministic public-Interface tests cover compatible/incompatible readiness, returned IDs, both providers, launch-stage failures, concurrent token isolation, byte-exact argv/env, fail-closed wrapper behavior, typed five-state prompt/wait outcomes, stall recovery/exhaustion, the 5000 ms boundary, bounded-cadence typed status reads with blocked/unknown/read-failure/stale/hard-budget supervision outcomes and preservation checkpoints, least privilege, cleanup, and obsolete-path deletion. Protocol reads replay committed real Herdr 0.7.5 responses carrying argv, stdout/stderr, exit status, timestamp, version, binary hash, and accurate redaction/parameterization provenance; a fidelity gate rejects derived envelopes, provenance loss, undeclared placeholders, or false status claims. The only executable double behavior is non-recordable launch/timing physics, checked by an opt-in real-vs-replay differential including process replacement, not-found, pane-busy, and stall/short-timeout behavior. A separate opt-in real-binary smoke proves the acknowledged launch chain and default-server non-interference. Run `make all` because this boundary affects shared delegation. | `Agent Fixes` for raw/decomposed input, direct provider worker launch, caller-substitutable worker profiles, unbounded client-side choreography outside the approved supervision contract, provider-specific prompt logic, widened worker authority, untyped native failures, incomplete deletion, or missing provider direction coverage. `Human Escalation` only when the pinned Herdr contract and live provider behavior irreconcilably conflict. |
...
| Irrecoverable runtime failure classification and escalation | Runtime retry policy uses a provider-neutral failure family before ordinary retry scheduling and an exact structural allowlist for recoverable provider rate-limit, capacity, service-availability, and network classes. Unknown failures fail closed; substring matches never make arbitrary timeouts retryable. Deterministic irrecoverable families immediately stop ordinary retry, verify and update same-scope process ownership to blocked, apply or preserve the exact `Human Escalation` tracker label, and move the issue to `Human Escalation` when supported. No Linear comment is required or created. Before the first equivalent redispatch, the queued observation must match an independently read durable ownership observation and current checkpoint; unchanged checkpoint plus identical fingerprint plus no new reset evidence blocks redispatch. A retry that later exits normally remains a typed blocked failure and does not erase durable failure evidence. Tracker mutation failures stay blocked and do not re-enter ordinary retry. Status API, dashboard, logs, and process ownership distinguish blocked failures from retry queue entries. Secret-bearing provider payloads, tokens, environment values, full issue bodies, and unbounded process output are never emitted. | Evidence discovery inventories bounded Octo-owned role-run/status logs, process-ownership records, tracker state/labels, local runtime state, redacted incident evidence, and safe local CLI probes when available. Deterministic public-seam tests cover provider-adapter normalization, all accepted failure families, exact `herdr_agent_status_timeout` typing, fail-closed unknowns and substring exclusions, first-equivalent-redispatch suppression and reset conditions, retried-success durability, AgentRunner exit normalization, Orchestrator escalation routing, tracker label/state success and failure paths, process-ownership status/API representation, compact run-log summaries, and redaction. Include existing Codex and Claude auth/no-progress regressions and at least one real adapter-boundary fixture. Mutation proof must preserve symbols and demonstrate behavioral RED at the public monitored-run seam. Run broader `make all` validation. | `Agent Fixes` for missing families, transient over-classification, retry leakage, failure-to-block the first equivalent redispatch, failed-run laundering into completion, delayed escalation, comment traffic, missing status distinction, tracker mutation regressions, redaction gaps, or fixtures that bypass provider parsing. `Human Escalation` when provider/runtime semantics conflict with durable sources or require operator-owned credential/configuration changes to validate. |
