...
| --- | --- | --- | --- |
| Current-run activity during slow routine persistence | Symphony Service 8.5.1 owns original-ingress monotonic freshness, exact-run rejection, serviceability in the bounded slow-write/burst case and preservation of existing critical accounting. Its TUR-878 scope correction excludes optional generalized backpressure hardening. Agent Runtime retains durable ownership and terminal fencing. No timeout-only repair or dequeue-time freshness. | TUR-878 requires RED/GREEN through real dispatch, AgentRunner, running Orchestrator and ProcessOwnership with a gated real filesystem primitive, two issues, a bounded burst and an interleaved tick. Add old-only backlog, stale replacement, same-run continuation, accounting/rate-limit, handoff-grace and terminal-write race cases. Assert real status, ownership, cleanup and zero Linear HTTP; preserve activity/accounting/retry/settlement suites and run exact-head `make all`. Callback-only or fake-owned-Module proof is insufficient. | `Agent Fixes` for stale-run mutation, starvation, clock/ordering drift, lost critical events, durability regression or missing faithful RED. `Human Escalation` for a genuine unresolved contract or required environment gap; historical incident causality remains qualified. |
...
| Explicit deployment host resources | An absent declaration is a no-op. Nonempty declarations are strict, local-only, generation-bound inputs resolved through `HostResourceContract` before provider or Herdr launch. Only assigned operation identity, exact commands, exact reads, and redacted provenance reach Codex participants; mixed-provider validation adds no Claude permission mode or gateway bypass. Unsafe roots, unsupported materialization, malformed or mismatched provenance, mutable launcher indirection, and missing expected context fail irrecoverably without ambient HOME/PATH discovery, installation, trust, or broader grants. | Public Config and Agent Runtime tests cover strict shape, direct-entry fail-before-launch behavior, ordinary and delegated projection, operation identity, redaction, path/type/hash/version/symlink failures, concurrent version isolation, and per-session revalidation. The opt-in real installed Codex sandbox proof must execute the selected mise/Elixir/Erlang commands, perform DNS, write a correlated worker event, prove negative denials, and tear down cleanly. Run focused tests, `mix specs.check`, and `make all`. | `Agent Fixes` for consumer, projection, isolation, redaction, or deterministic-proof gaps. `Human Escalation` when authoritative producer context is missing or conflicting, or the required real sandbox cannot exercise installed resources and DNS without widening policy. |
