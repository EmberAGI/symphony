...
defmodule SymphonyElixir.TestSupport do
  ...
  defp workflow_content(overrides) do
    config =
      Keyword.merge(
        [
          ...
          # Bounded test override of the production 30s total tracker-request
          # deadline: no suite request may stall longer than this.
          tracker_request_timeout_ms: 5_000,
          ...
          telegram_message_thread_id: nil,
          telegram_events: nil,
    ...
    tracker_request_timeout_ms = Keyword.get(config, :tracker_request_timeout_ms)
    ...
    telegram_message_thread_id = Keyword.get(config, :telegram_message_thread_id)
    ...
    sections =
      [
        ...
        "  request_timeout_ms: #{yaml_value(tracker_request_timeout_ms)}",
  ...
  defp notifications_yaml(nil, nil, nil, nil, nil), do: nil

  defp notifications_yaml(endpoint, bot_token, chat_id, message_thread_id, events) do
    [
      "notifications:",
      "  telegram:",
      ...
      message_thread_id && "    message_thread_id: #{yaml_value(message_thread_id)}",
