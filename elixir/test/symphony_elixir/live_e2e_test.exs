defmodule SymphonyElixir.LiveE2ETest do
  ...
  defp receive_runtime_info!(issue_id) do
    receive do
      {:worker_runtime_info, ^issue_id, _envelope, %{workspace_path: workspace_path} = runtime_info}
      when is_binary(workspace_path) ->
        runtime_info

      {:codex_worker_update, ^issue_id, _envelope, _message} ->
  ...
  defp run_live_issue_flow!(backend) when backend in [:local, :ssh] do
    ...
    try do
      ...
      assert :ok = run_agent_with_ownership(issue, self(), max_turns: 3)
