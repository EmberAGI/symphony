...
| Duplicate top-level role-run prevention | Top-level implementer/reviewer/QA/etc. dispatch must be gated by all same-scope Linear-visible structured claim leases plus local process ownership metadata. Same-thread continuation inside one role run must preserve `agent.max_turns` behavior. Retry-by-ID and leaked-claim cleanup regressions must stay covered. Process cleanup/quarantine tests must use synthetic fixtures or controlled local PIDs, never committed production process registries, generated workspaces, or session logs. | Deterministic Elixir tests covering claim lease marker parsing and upsert ownership, duplicate dispatch refusal, coexistence of different-role/workspace leases and process records without hiding same-scope active owners, expired lease recovery when no live process remains, retry-by-ID preservation, leaked-claim cleanup preservation, snapshot/API lease and process payloads, and a synthetic process-leak fixture showing a live child PID blocks replacement dispatch. Run broader `make all` validation because this surface changes shared orchestration and status behavior. | `Agent Fixes` for missing duplicate-refusal, retry/recovery regressions, noisy tracker updates, or insufficient process fixture proof. `Human Escalation` when required tracker/process semantics conflict with issue or ADR sources. |
