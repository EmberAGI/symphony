...
| --- | --- | --- | --- |
| Herdr delegation turn submission | Herdr remains the deep module that owns atomic terminal input and provider keyboard-protocol semantics. Symphony must submit orchestrator assignments, worker results, and consultation responses through one unchanged `pane run`; transport Adapters and generated launchers must not branch on Codex, Claude Code, or another provider to reproduce the operation with `send-text`, synthesized keys, or multiple PTY writes. Turn lifecycle separately requires a post-submission `working` observation before idle/done completion so stale idle cannot masquerade as a completed turn. | Deterministic transport contract tests covering the initial orchestrator assignment and both directions of worker consultation, proving each uses one `pane run` and no `send-text`; lifecycle tests covering working acknowledgement and stale-idle refusal; and a bounded isolated live Claude test showing multiline `pane run` transitions idle to working to idle without touching the operator's default Herdr server. Run broader `make all` validation because this boundary affects shared delegation. | `Agent Fixes` for provider-specific PTY input logic, decomposed submission, missing direction coverage, or stale-idle completion. `Human Escalation` only when the pinned Herdr contract and a live provider behavior irreconcilably conflict. |
